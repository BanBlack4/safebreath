...
interface CalmInterventionScreenProps {
  ...
  onSetIncidentAlerts: React.Dispatch<
    React.SetStateAction<
      { name: string; type: "sms" | "call"; timestamp: string }[]
    >
  >;
...
export default function CalmInterventionScreen({
  ...
  const [phase, setPhase] = useState<
    "prompt" | "breathing" | "escalation" | "sos_dispatch"
  ...
  const initiateCall = (name: string, phone: string, isOfficial = false) => {
    ...
    // Track sent alert
    const currentMins = new Date().toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    });
    setAlertsSent((prev) => {
  ...
  const contactsKey = (profile.emergencyContacts || [])
    .map((c) => `${c.id}`)
    .join(",");
  useEffect(() => {
    if (phase === "sos_dispatch") {
      ...
      const messages = [
        ...
        ...emergencyContacts.map(
          (c) =>
            `▶ Enviando alerta SMS a ${c.name} (${c.countryCode || "+56"} ${c.phone})`,
        ),
  ...
  const initialMotion = reducedMotion
    ? { opacity: 0 }
    : { opacity: 0, scale: 0.95, y: 15 };
  const animateMotion = reducedMotion
    ? { opacity: 1 }
  ...
  return (
    <div
      className={`flex flex-col h-full p-5 pb-28 justify-between items-center fixed inset-0 z-30 overflow-y-auto transition-colors duration-1000 ${bgStyle}`}
    >
      {/* Background Soft Gradients */}
      <div
        className={`absolute inset-0 bg-gradient-to-br from-[#0f172a] via-[#020617] to-[#042f2e] pointer-events-none transition-opacity duration-1000 ${ambientGradientOpacity}`}
      />
        ...
        <div className="w-full max-w-md px-2 mt-3 z-15 animate-fade-in">
          <div className="bg-emerald-950/60 backdrop-blur-sm border border-emerald-500/30 rounded-2xl p-3 flex items-center justify-between text-xs shadow-lg">
            <div className="flex items-center gap-2 text-emerald-200">
              ...
              <span className="font-sans font-medium text-emerald-200">
                Alertas activas ({alertsSent.length} contacto
                {alertsSent.length > 1 ? "s" : ""})
              </span>
            </div>
            {phase !== "sos_dispatch" ? (
              <button
              ...
              <span className="text-[9px] font-mono uppercase bg-emerald-900/40 text-emerald-300 px-2.5 py-1 rounded-lg border border-emerald-800/50">
                Monitoreando
              </span>
            )}
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "prompt" && (
            ...
            >
              ...
              <h1
                className={`text-3xl font-display font-medium tracking-tight mb-3 ${nightMode ? "text-slate-300" : "text-[#e0f2fe]"}`}
              >
                ¿Te sientes bien?
              </h1>

              <p
                className={`text-sm mb-6 font-light leading-relaxed ${nightMode ? "text-slate-400" : "text-[#99f6e4]"}`}
              >
                Detectamos una velocidad de {profile.bpmReposo + 45} BPM en tu
                pulsación. Es posible que sea agitación física, pero si es un
                momento estresante, te podemos guiar a regularlo de inmediato.
              </p>
              ...
              <div className="border border-white/5 bg-slate-950/30 rounded-2xl p-4 text-left space-y-2">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      ¿Por qué esta pantalla no es una alarma ruidosa?
                    </span>
                  </div>
                  {showExplanation ? (
                ...
                {showExplanation && (
                  ...
                  >
                    <p className="text-[11px] text-slate-400 leading-relaxed font-light">
                      La bio-retroalimentación demuestra que lanzar alarmas
                      estrepitosas de "¡EMERGENCIA!" cuando una persona tiene
                      pulso elevado provoca una{" "}
                      <strong>amplificación catastrófica del pánico</strong> (un
                      bucle de retroalimentación en el que el miedo acelera el
                      corazón aún más).
                    </p>
                    <p className="text-[11px] text-slate-400 leading-relaxed font-light mt-1.5">
                      SafeBreath te ofrece un canal respetuoso, dándote opción
                      de <strong>respirar</strong> primero, o descartar como{" "}
                      <strong>falsa alarma</strong> (por ejercicio o
                      calibración). Si no respondes o lo requieres, los canales
                      de ayuda con tus contactos salvaguardan tu integridad
                      física de inmediato.
                    </p>
                  </motion.div>
                )}
          ...
          {phase === "breathing" && (
            ...
            >
              ...
              <div className="flex gap-2 text-xs font-mono text-slate-500 justify-center">
                <span
                  className={
                    breathingStep === "inhale" ? "text-[#2dd4bf] font-bold" : ""
                  }
                >
                  4s Inhalación
                </span>
                ...
                <span
                  className={
                    breathingStep === "hold" ? "text-[#2dd4bf] font-bold" : ""
                  }
                >
                  4s Contener
                </span>
                ...
                <span
                  className={
                    breathingStep === "exhale" ? "text-[#2dd4bf] font-bold" : ""
                  }
                >
                  8s Exhalación
                </span>
              ...
              <div className="mt-8 text-xs text-slate-400 w-full max-w-sm leading-relaxed p-3 bg-white/5 rounded-xl border border-white/5">
                "Sincroniza tu pecho con el domo de luz. Esta respiración
                estimula el nervio vago bajando el ritmo simpático de forma
                involuntaria."
              </div>
            </motion.div>
          )}
          ...
          {phase === "escalation" && (
            ...
            >
              ...
              <p className="text-sm text-slate-400 mb-8 max-w-sm">
                Completamos la pauta de respiración, pero tu ritmo sigue
                elevado. Si necesitas alertar a tus contactos de emergencia
                inmediatos, utiliza el deslizador a continuación.
              </p>
          ...
          {phase === "sos_dispatch" && (
            ...
            >
              ...
              <p className="text-xs text-slate-350 mb-6 text-center leading-relaxed max-w-sm">
                Toma una respiración profunda. Si lo necesitas, puedes realizar
                una llamada telefónica o enviar un mensaje rápido para recibir
                ayuda.
              </p>
              ...
              <div className="w-full space-y-3 mb-4 text-left font-sans">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest pl-1">
                  Canales de Auxilio (Vocal y Silencioso)
                </h3>
                {/* 911 Call Button */}
                <button
                  onClick={() => {
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-extrabold text-xs uppercase tracking-wider text-red-200">
                        LLAMAR AL 911
                      </p>
                      <p className="text-[10px] text-slate-400 font-mono text-xs">
                        Servicio Público de Rescate
                      </p>
                    </div>
                  </div>
                  <span className="text-[10px] font-bold px-2 py-0.5 bg-red-650 rounded-md text-white font-mono uppercase">
                    Vocal
                  </span>
                </button>
                ...
                {emergencyContacts.length === 0 ? (
                  <div className="text-center py-4 bg-white/5 rounded-2xl border border-dashed border-white/10">
                    <p className="text-xs text-slate-400">
                      No registraste contactos de emergencia en tu Perfil de
                      Salud. El sistema no pudo auto-alertar parientes.
                    </p>
                  </div>
                ) : (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-semibold">
                              {contact.relation} •{" "}
                              {contact.countryCode || "+56"} {contact.phone}
                            </p>
                          </div>
                        </div>
                        {alertsSent.some((a) => a.name === contact.name) ? (
                          <span className="text-[9px] font-mono uppercase bg-emerald-950/80 text-emerald-300 px-2.5 py-1 rounded border border-emerald-800/80 flex items-center gap-1.5 font-bold">
                            ...
                            Alertado (
                            {alertsSent.find((a) => a.name === contact.name)
                              ?.type === "sms"
                              ? "SMS"
                              : "Llamada"}
                            )
                          ...
                          <span className="text-[9px] font-mono uppercase bg-rose-950/60 text-rose-300 px-2 py-0.5 rounded border border-rose-900">
                            Contacto
                          </span>
                        )}
                      </div>
      ...
      <div className="w-full pb-6 flex flex-col items-center z-10 max-w-md">
        <AnimatePresence>
          ...
          {phase === "escalation" && (
            ...
            >
              <div
                className={`relative w-full h-16 rounded-2xl flex items-center justify-center overflow-hidden shadow-[0_0_20px_rgba(239,68,68,0.2)] ${nightMode ? "bg-[#3f0712] border border-red-950" : "bg-[#7f1d1d] border border-red-800"}`}
              >
                <span
                  className={`absolute inset-x-0 w-full text-center font-bold tracking-widest uppercase text-xs pointer-events-none mx-auto opacity-90 pl-8 ${nightMode ? "text-red-300" : "text-white"}`}
                >
                  Desliza SOS &rarr;
                </span>
                <input
          ...
          >
            ...
            <div className="w-full flex flex-col items-center pt-16 z-10 text-center">
              <span className="text-[10px] font-bold tracking-widest text-[#2dd4bf] uppercase font-mono bg-teal-950/60 px-3 py-1.5 rounded-full border border-teal-800/40 animate-pulse flex items-center justify-center gap-1.5 mb-8 mx-auto">
                ...
                {activeCall.isOfficial ? "Oficial" : "Personal"}
              </span>
            ...
            <div className="w-full max-w-sm mx-auto bg-slate-900/60 border border-white/5 rounded-3xl p-5 text-left space-y-2 z-10 backdrop-blur-sm">
              ...
              <p className="text-[11px] text-slate-400 leading-relaxed font-light font-sans">
                Para garantizar la integridad técnica y evitar que el visor web
                se congele o te expulse del entorno de diseño, iniciamos este
                dialer de asistencia táctil integrado.
              </p>
              <p className="text-[11px] text-slate-400 leading-relaxed font-light font-sans bg-white/5 p-2 rounded-xl border border-white/5">
                En tu smartphone iOS o Android, SafeBreath lanza el
                intercomunicador nativo para llamar al contacto real usando tu
                plan de datos sin costes extras de red de servidor.
              </p>
            </div>
        ...
        {activeMessage && (
          ...
          >
                ...
                <div className="flex justify-between items-center pb-2 border-b border-white/5">
                  ...
                  {activeMessage.status === "sending" && (
                    <span className="flex items-center gap-1 text-[10px] text-teal-400 font-mono animate-pulse">
                      <Loader2 className="w-3 h-3 animate-spin" /> Despachando
                      satélite...
                    </span>
                  )}
            ...
            <div className="w-full max-w-sm mx-auto bg-[#1e293b]/30 border border-white/5 rounded-2xl p-4 text-left z-10">
              <p className="text-[11px] text-slate-400 leading-relaxed font-light">
                ...
                <strong className="text-slate-200">
                  ¿Por qué usar Mensajería Silenciosa?
                </strong>{" "}
                Durante crisis severas de asma, dolores pectorales agudos u
                ataques de pánico violentos con hiperventilación, el usuario es
                físicamente{" "}
                <strong className="text-[#2dd4bf]">
                  incapaz de pronunciar palabras
                </strong>
                . El mensaje silencioso de un solo toque pre-comprime toda la
                biometría crítica para tu pariente.
              </p>
            </div>
            ...
            <div className="w-full max-w-sm mx-auto pt-6 pb-8 z-10">
              ...
              >
                {activeMessage.status === "delivered"
                  ? "✓ Volver a Coordinación de S.O.S."
                  : "Cancelar Envío"}
              </button>
            </div>
          </motion.div>
